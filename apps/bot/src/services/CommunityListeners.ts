...
async function handleStarboard(reaction: MessageReaction, client: Client): Promise<void> {
  ...
  const guildId = msg.guildId ?? reaction.message.guildId;
  if (!guildId) return;
  ...
  if (existing) {
    const posted = await starCh.messages.fetch(existing.starboardMessageId).catch(() => null);
    if (posted) {
      await posted.edit({ embeds: [embed] }).catch(() => undefined);
      return;
    }
  }
  const sent = await starCh.send({ embeds: [embed] }).catch(() => null);
  if (!sent) return;
  await prisma.starboardPost
    .upsert({
      where: { guildId_sourceMessageId: { guildId, sourceMessageId: msg.id } },
      create: { guildId, sourceMessageId: msg.id, starboardMessageId: sent.id },
      update: { starboardMessageId: sent.id },
    })
    .catch(() => undefined);
}
