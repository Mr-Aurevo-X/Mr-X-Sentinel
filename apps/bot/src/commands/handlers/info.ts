...
export async function handleStats(interaction: ChatInputCommandInteraction): Promise<CommandReply> {
  ...
  return {
    embeds: [
      buildSimpleEmbed(
        "Statistiques",
        [
          `Serveur : **${guild.name}** · **${guild.memberCount}** membres`,
          `Salons : **${guild.channels.cache.size}** · Rôles : **${guild.roles.cache.size}**`,
          `Bot : **${client.guilds.cache.size}** serveurs · **${mem.toFixed(1)}** Mo RAM`,
